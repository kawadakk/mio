...
#![allow(dead_code)]
pub use libc::{
    c_char, c_int, c_long, c_short, c_uint, c_void, size_t, ssize_t, suseconds_t, time_t, timeval,
};
...
pub type EventFlags = c_int;
pub const EFD_NONBLOCK: EventFlags = 1;

pub type PollMask = c_short;
/// non-high-priority read
pub const EPOLLIN: PollMask = 0x1;
/// normal write
pub const EPOLLOUT: PollMask = 0x2;
/// error (only valid in revents)
pub const EPOLLERR: PollMask = 0x4;
/// invalid FD (only valid in revents)
pub const EPOLLNVAL: PollMask = 0x8;
/// normal read
pub const EPOLLRDNORM: PollMask = 0x10;
/// priority read
pub const EPOLLRDBAND: PollMask = 0x20;
/// high-priority read
pub const EPOLLPRI: PollMask = 0x40;
/// equivalent to POLLOUT
pub const EPOLLWRNORM: PollMask = 0x80;
/// priority write
pub const EPOLLWRBAND: PollMask = 0x100;
/// hangup (only valid in revents)
pub const EPOLLHUP: PollMask = 0x200;

pub type EPollFdFlags = u16;
/// edge-triggered
pub const EPOLLET: EPollFdFlags = 1;
/// one-shot
pub const EPOLLONESHOT: EPollFdFlags = 2;
...
pub struct epoll_event {
    /// watched or received events
    pub events: PollMask,
    /// epoll mode; ignored when waiting
    pub flags: EPollFdFlags,
    /// user data
    pub u64: u64,
}

pub type EPollFlags = c_int;

pub type EPollOp = c_int;
pub const EPOLL_CTL_ADD: EPollOp = 1;
pub const EPOLL_CTL_MOD: EPollOp = 2;
pub const EPOLL_CTL_DEL: EPollOp = 3;
...
extern "C" {
    ...
    #[link_name = "SOLID_NET_Event"]
    pub fn eventfd(init: u64, flags: EventFlags) -> c_int;

    #[link_name = "SOLID_NET_Fcntl"]
    pub fn fcntl(fd: c_int, cmd: c_int, val: c_int) -> c_int;

    #[link_name = "SOLID_NET_EPollCreate"]
    pub fn epoll_create(flags: EPollFlags) -> c_int;

    #[link_name = "SOLID_NET_EPollCtl"]
    pub fn epoll_ctl(epfd: c_int, op: EPollOp, fd: c_int, event: *const epoll_event) -> c_int;

    #[link_name = "SOLID_NET_EPollWait"]
    pub fn epoll_wait(
        fd: c_int,
